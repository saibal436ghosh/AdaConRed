"""
Split conformal prediction (LAC) with conformal redistribution.

Builds standard split-conformal prediction sets by thresholding individual
class probabilities, then applies the label-free redistribution rule.

The proposed entropy-modulated, margin-aware score is implemented
separately. The redistribution rule is identical in both, so differences in
refined accuracy are attributable to set construction alone.
...
probs = softmax_calib
true_probs = probs[np.arange(len(y_calib)), y_calib]
...
prediction_sets = [
    np.where(probs_test[i] >= threshold)[0]
...
# Build conformal prediction sets (sorted)

sorted_indices = np.argsort(-probs_test, axis=1)

prediction_sets_sort = [
    idxs[probs_test[i, idxs] >= threshold]
    for i, idxs in enumerate(sorted_indices)
]

prediction_sets_sort = np.array(prediction_sets_sort, dtype=object)
...
# Average set size (coverage is computed in the conformal metrics block below)
...
covered_samples = np.sum(coverage_list)
total_samples = len(y_test)

print("\nOverall Coverage (conformal):", coverage)
print(f"Out of {total_samples} samples, {covered_samples} samples are covered")

# Class-wise coverage
classwise_coverage = {}

print("\nClass-wise Coverage (conformal):")
...
for c in range(len(class_names)):
    ...
    covered_c = np.sum(coverage_list_c)
    total_c = len(idx)

    cov_c = covered_c / total_c
    classwise_coverage[class_names[c]] = cov_c

    print(f"{class_names[c]}: {cov_c:.4f} "
          f"(Out of {total_c}, {covered_c} covered)")
...
for i in range(len(preds)):
    ...
    # --- Label-free gate  ---
    # Depends only on the point prediction and the cardinality of the
    ...
    # --- Reduced set ---
    # Gamma'(x) = Gamma(x) \ {AMB}; the set is already sorted by descending
